...
export default function Stok() {
  ...
  return (
    <div className="w-full">
      {/* Header */}
      <div className="mb-21 lg:mb-34">
        <h1 className="text-xl font-black text-slate-800 tracking-tight">Penyesuaian Stok</h1>
        <p className="text-xs text-slate-500 mt-2">Adjustment otomatis masuk / keluar log</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-21 lg:gap-34">
        ...
        <div className="w-full lg:w-1/3 shrink-0">
          {/* Form Card */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-21 lg:sticky lg:top-[104px]">
            <form onSubmit={handleSubmit} className="space-y-13">
              ...
              {selected && (
                <div className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-xl px-13 py-13">
                  <span className="text-sm font-semibold text-slate-500">Stok Saat Ini</span>
                  <span className="text-2xl font-black text-slate-800">{selected.stok_saat_ini}</span>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-8">
                  {['masuk', 'keluar'].map(j => (
                    <button key={j} type="button"
                      onClick={() => setJenis(j)}
                      className={`flex items-center justify-center gap-8 py-8 rounded-xl text-sm font-semibold
                    ...
                    >
                      {j === 'masuk' ? <ArrowDown size={14} /> : <ArrowUp size={14} />}
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-8">Masukkan Jumlah</p>
                <input
                  type="number" min="1" required
                  placeholder="0"
                  value={jumlah}
                  onChange={e => setJumlah(Number(e.target.value))}
                  className="w-full px-13 py-13 text-xl font-bold rounded-xl border border-slate-200
                    focus:border-teal-500 focus:ring-2 focus:ring-teal-100 outline-none bg-white transition-all"
                />
              </div>

              <Button type="submit" className="w-full justify-center py-13 bg-teal-600 hover:bg-teal-700 text-white shadow-sm mt-5">
        ...
        {/* BAGIAN KANAN: Daftar Stok (Lebar 2/3 di desktop) */}
        <div className="w-full lg:w-2/3 flex-1">
              ...
              <div className="flex gap-8">
                <div className="relative flex-1">
                  ...
                  <input 
                    ...
                    className="w-full pl-34 pr-13 py-8 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:border-teal-500 focus:ring-2 focus:ring-teal-100 outline-none transition-all"
                  />
                </div>
                <button className="w-34 h-34 flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl transition-colors shrink-0" title="Scan Barcode">
                  <QrCode size={16} />
                </button>
              </div>
          ...
          {filteredBarang.length === 0 ? (
            <div className="text-center py-34 text-slate-400 text-sm bg-white rounded-2xl border border-slate-200 border-dashed">
